...
public class PlayerMovement : MonoBehaviour
{
    ...
    [SerializeField] private float vSpeedMax;

    private enum MovementState { idle, running, jumping, falling, doubleJumping, prout, attack }

    [Header("Sound")]
    [SerializeField] private AudioSource jumpSoundEffect;
    [SerializeField] private AudioSource fartEffect;

    [SerializeField] private GameObject go;

    [Header("icy")]
    [SerializeField] public bool ice;
    ...
    private void Update()
    {
        ...
        {
            rb.velocity = new Vector2(rb.velocity.x, -vSpeedMax);
        }
        ...
        if (ice)
        {
            if (dirX != 0)
            {
                rb.velocity = Vector2.SmoothDamp(rb.velocity, new Vector2(dirX * moveSpeed, rb.velocity.y), ref currentVelocity, friction);
            }
            else
            {
                rb.velocity = Vector2.SmoothDamp(rb.velocity, new Vector2(0, rb.velocity.y), ref currentVelocity, friction * 5);
            }
        }
        else
        {
    ...
    private void UpdateAnimationState()
    {
        ...
        else
        {
            ...
            else
            {
                ...
                else if (go.GetComponent<Animator>().GetCurrentAnimatorStateInfo(0).IsName("Player1_Prout"))
                {
                    return;
                }
                else
